...
Only focus on the predictors: 12 continuous predictors and 5 categorical predictors.
...
Since our data is mixed with continuous variables and binary variables, so we cannot use Euclidean distance, Hanhattan distance and Minkowski distance to measure the dissimilarity. One way is to use Gower similarity (𝐺𝑆).
```{r}
library(cluster)
```

```{r}
g.dist = daisy(dat, metric="gower", type=list(symm=2))
