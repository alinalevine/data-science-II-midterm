...
```{r}
full_data = 
  plyr::join_all(list(demographics, income, depression, diabetes, smoking, diet_behavior, alcohol, medical_cond, coronary), by = "seqn", type = "inner") %>% 
...
full_data = 
  full_data %>%
  mutate(citizen = as.factor(ifelse(2, 0, citizen))) 

  
full_data_binary_subset = 
  full_data %>% 
  dplyr::select(c(gender, citizen, diabete, smoked, drinker, heavy_drinker, overweight, gout, disease))

full_data_non_binary = 
  full_data %>%
  dplyr::select(-c(gender, citizen, diabete, smoked, drinker, heavy_drinker,overweight,gout, disease))

full_data = 
  cbind(map_dfc(full_data_binary_subset, ~as.factor(ifelse(.x==2, 0, .x))), full_data_non_binary) %>%
